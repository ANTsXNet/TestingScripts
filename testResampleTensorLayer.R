...
source <- layer_input( shape = list( NULL, NULL, 1 ) )
target <- layer_input( shape = list( NULL, NULL, 1 ) )

sourceImage <- splitChannels( antsImageRead( bikeFile ) )[[1]]
targetImage <- as.antsImage( array( data = 0, dim = c( targetShape ) ) )

sourceArray <- array( data = as.array( sourceImage ), dim = c( 1, dim( sourceImage ), 1 ) )
...
for( i in seq_len( length( types ) ) )
  ...
  output <- source %>% layer_resample_tensor_to_target_tensor_2d( target, types[i] )
  ...
  # batchX <- array( data = f, dim = c( 1, dim( f ) ) )
  # outputImage <- drop( model %>% predict( list( batchX ) )
